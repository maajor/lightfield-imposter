...
def main(max_level=3):
    ...
    colors = colors[:,:,:,0] # only use one channel
    ...
    for lmax in range(0, max_level):
        ...
        data_path = "dataset/3d_imposter_sh_{0}.npy".format(lmax)
        if os.path.exists(data_path):
            coeffs = np.load(data_path)
        else:
            coeffs = sample_sh(colors=colors, coords=coords, lat_step=180/16, long_step=360/16, lmax=lmax)
            np.save("dataset/3d_imposter_{0}".format(lmax), coeffs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--maxlevel", type=int, help="pixel x index", default=10)
